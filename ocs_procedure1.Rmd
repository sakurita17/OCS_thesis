---
title: "Optimum Contribution Selection - OCS"
...
## Data description
Pedigree
animal: Integer variable. Identification of each animal.
sire: Integer variable. Identification number of the father. 
dam: Integer variable. Identification number of the mother. 


geb_dat: date variable. Date of birth.
sex: Sex column. 1:Male or 2:Female.
Basic_Animal:	Character. Identification of base animals.
NBA: Numeric variables. Number of born alive (number of piglets).	
DailyGain: Numeric variable. Daily gain (gr/day).	
Backfat: Numeric variable. Back fat (mm)	
sem: Numeric variable. Birth semester. Min 19982 to 20182.	
new_id: Integer variable. Consecutive number to run EVA.	
par20182: Integer variable. Parents of animals born in 20182.	
maxunic: Numeric variable. maxmating's number. 1:female and 20:male.
index:	Numeric variable. selection index, each trait was weighted by 1/3. 
cohort:	Numeric variable. Cohort based on  birth semester. i.e., cohort 1:19982
cohort 41:20182.
```{r}
ped = readRDS("D:/Masterthesis/R/output/ped_reduced.rds") # import "ped" as .rd
head(ped)
```
## Plots number of animals
I will plot the number of animals per cohort and the number of founder animal per cohort.
...
#ggsave(
#  filename="D:/Masterthesis/graphs/tot_found.png",width=10,height=2*3,dpi=300)

```
## Quality of the pedigree
The quality of the pedigree was measured by pedigree completeness index (PCI) and complete generation equivalent (EqGi). Those values PCI and EqGi were computed in EVA.
```{r}
...
ggarrange(EQ,PCI, labels = c("a","b"), ncol = 1, nrow = 2,
          common.legend = TRUE, legend = "bottom")
#ggsave(
#  filename="D:/Masterthesis/graphs/PCI_Eq.png",width=10,height=2*3,dpi=300)
```
## Generation time
The genetic response to selection predicts how much better the next generation performs compared to the current generation. T_g  was computed as the mean of the mean age of dams and boars born in particular year.
```{r}
# boars_age
ped$age_b <- NA
for (i in 1:nrow(ped)) {
...
#ggsave(
#  filename="D:/Masterthesis/graphs/tg.png",width=10,height=2*3,dpi=300)
```
## Average inbreeding, average coancestry and Fis
These values were computed in EVA.
```{r}
...
#ggsave(
#  filename="D:/Masterthesis/graphs/inbreeding_coancestry.png",
#  width=10,height=1*3,dpi=300)
```
## Change in Fst
The change in inbreeding Fst was plotted to identify changes in the inbreeding trend in the original population.
```{r}
...
#ggsave(
#  filename="D:/Masterthesis/graphs/drift_coeff.png",width=10,height=2*3,#dpi=300)

```
## Effective population size
Based on the changes of Fst, the before and after cohort 23 (second half of 2009).
```{r}
...
uno <- candidates_list[[1]]
uno <- uno[uno$Sex == 1 & uno$N.matings != 0,]
summary(uno)
sd(uno$Rel.males.sel)

dos <- candidates_list[[2]]
dos <- dos[dos$Sex == 1 & dos$N.matings != 0,]
sd(dos$Merit)
sd(dos$Rel.males.sel)

tres <- candidates_list[[3]]
tres <- tres[tres$Sex == 1 & tres$N.matings != 0,]
sd(tres$Merit)
sd(tres$Rel.males.sel)

cuatro <- candidates_list[[4]]
cuatro <- cuatro[cuatro$Sex == 1 & cuatro$N.matings != 0,]
sd(cuatro$Merit)
sd(cuatro$Rel.males.sel)

cinco <- candidates_list[[5]]
cinco <- cinco[cinco$Sex == 1 & cinco$N.matings != 0,]
sd(cinco$Merit)
sd(cinco$Rel.males.sel)

seis <- candidates_list[[6]]
seis <- seis[seis$Sex == 1 & seis$N.matings != 0,]
sd(seis$Merit)
sd(seis$Rel.males.sel)

siete <- candidates_list[[7]]
siete <- siete[siete$Sex == 1 & siete$N.matings != 0,]
sd(siete$Merit)
sd(siete$Rel.males.sel)

complete <- dplyr::bind_rows(
  list(uno = uno, dos=dos, tres = tres,
       cuatro = cuatro, cinco = cinco,
       seis = seis, siete = siete),
  .id = "source"
)

cand <- merge(uno[,c(1,3)],dos[,c(1,3)], by = c("ID"), all = TRUE)
cand2 <- merge(tres[,c(1,3)],cuatro[,c(1,3)], by = c("ID"), all = TRUE)
cand3 <- merge(cinco[,c(1,3)],seis[,c(1,3)], by = c("ID"), all = TRUE)
cand <- merge(cand,siete[,c(1,3)], by = c("ID"), all = TRUE)
cand2 <- merge(cand2, cand3, by = c("ID"), all = TRUE)
cand <- merge(cand, cand2, by = c("ID"), all = TRUE)
colnames(cand) <- c("id","uno","dos","siete","tres",
                    "cuatro","cinco","seis")
cand$index <- ped[ped$animal %in% unique(cand$id),]$index
summary(cand)

#write.xlsx(cand, file = "D:/Masterthesis/graphs/cand.xlsx", 
#           sheetName="cand", append=TRUE)
```
...
#ggpubr::ggarrange(plt_nba,plt_dg,plt_bf,
#                 labels = c("a","b","c"),heights = c(1, 1), ncol = 1, nrow = 3,
#                  common.legend = TRUE, legend = "bottom")
```
## Function trait plot - summary

```{r}
## Data scaling
born20182 <- ped[ped$sem == 20182,]
# without mean all population
ped$NBA_scal <- 
  (ped$NBA)/sd(ped[ped$animal%in%unique(c(born20182$sire,born20182$dam)),]$NBA)
ped$Daygain_scal <- 
  (ped$DailyGain)/sd(ped[ped$animal%in%unique(c(born20182$sire,born20182$dam)),]$DailyGain)
ped$Backfat_scal <- 
  (ped$Backfat)/sd(ped[ped$animal%in%unique(c(born20182$sire,born20182$dam)),]$Backfat)
...
ggpubr::ggarrange(plot1,plot2,
                  ...
                  common.legend = TRUE, legend = "bottom")
#ggsave(filename=
#    "D:/Masterthesis/graphs/before_after_scaling1.png",
#  width=13,height=2*3,dpi=300)
...
# Upset plot function (author)
```{r}
######## Comparison of the mating 
# list of unique matings or males
# path where the plot is going to stay
# x = vector of breaks for the bar plot, stating with 0
...
```
## List number of mating per selected boar 
...
index <- unique(c(ocs_output_matings[[1]]$Sire,
                  ocs_output_matings[[2]]$Sire,
                  ocs_output_matings[[3]]$Sire,
                  ocs_output_matings[[4]]$Sire,
                  ocs_output_matings[[5]]$Sire,
                  ocs_output_matings[[6]]$Sire,
                  ocs_output_matings[[7]]$Sire))

index <- ped[ped$animal %in% unique(c(ocs_output_matings[[1]]$Sire,
                                      ocs_output_matings[[2]]$Sire,
                                      ocs_output_matings[[3]]$Sire,
                                      ocs_output_matings[[4]]$Sire,
                                      ocs_output_matings[[5]]$Sire,
                                      ...
                                      ocs_output_matings[[7]]$Sire))
             ,c(1,14)]
colnames(index) <- c("Var1","index")

contribution <- Reduce(function(...) merge(..., by = c('Var1'), all=TRUE), list(
  index,
  data.frame(table(ocs_output_matings[[1]]$Sire)),
  data.frame(table(ocs_output_matings[[2]]$Sire)),
  data.frame(table(ocs_output_matings[[3]]$Sire)),
  data.frame(table(ocs_output_matings[[4]]$Sire)),
  data.frame(table(ocs_output_matings[[5]]$Sire)),
  data.frame(table(ocs_output_matings[[6]]$Sire)),
  data.frame(table(ocs_output_matings[[7]]$Sire))))

colnames(contribution) <- c("boar","index","(1,0)","(1,-100)","(1,-300)",
                            "(1,-1000)",("1,-3000"),"(1,-10000)",
                            "(0,-1)")

contribution <- contribution[order(-contribution$index),]
```

## genetic trend selected boars
```{r}
boar_trend <-  function(list,ped){
  ...
  bf_sd <- rep(NA,7)
  for (i in 1:7) {
    nba_mean[i] <- mean(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$NBA)
    nba_sd[i] <- sd(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$NBA)
    
    dg_mean[i] <- mean(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$DailyGain)
    dg_sd[i] <- sd(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$DailyGain)
    
    bf_mean[i] <- mean(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$Backfat)
    bf_sd[i] <- sd(ped[ped$animal %in% unique(ocs_output_matings[[i]]$Sire),]$Backfat)
  }
  
  boar_trend <- data.frame(
    nba_mean, nba_sd, dg_mean, dg_sd, bf_mean, bf_sd 
  )
  print(boar_trend)
}

boar_trend(matings_output, ped)
...
## Rate of inbreeding and change in average coancestry
...
ggsave(filename='D:/Masterthesis/graphs/delta_rel_index_full.png',
       width=10,
       height=2*3,
       dpi=300)
...
## Differential selection per average coancestry
...
#ggsave(
#  filename=
#    "D:/Masterthesis/graphs/diff.sel.per.coanc.png",
#  width=15,height=2*3,dpi=300)
