...
I will plot the number of animals per cohort and the number of founder animal
per cohort.
...
The quality of the pedigree was measured by pedigree completeness index (PCI) and
complete generation equivalent (EqGi). Those values PCI and EqGi were computed in EVA.
```{r, results="hide", fig.show="hide", message = FALSE, warning = FALSE}
...
```
## Generation time
The genetic response to selection predicts how much better the next generation
performs compared to the current generation. Tg  was computed as the mean of the
mean age of dams and boars born in particular year.
```{r, results="hide", fig.show="hide", message = FALSE, warning = FALSE}
# boars_age
ped$age_b <- NA
for (i in 1:nrow(ped)) {
...
```
## Change in FST
The change in inbreeding Fst was plotted to identify changes in the inbreeding 
trend in the original population.
```{r, results="hide", fig.show="hide", message = FALSE, warning = FALSE}
...
#                  common.legend = TRUE, legend = "bottom")
```
## Scaling traits units
Breeding values on the natural (phenotypic) scale were provided, NBA in units of piglets, back fat thickness in millimetres and daily gain in units of grams of gain per day. Means and standard deviations of each breeding value within each cohort were estimated. 
```{r, results="hide", fig.show="hide", message = FALSE, warning = FALSE}
...
```
## Uncertainty
One scenario was arbitrarily selected to quantify the uncertainty of the method, the scenario was run five time in EVA, two variables were compared; EBV and F of the offspring from mating between sire and dam -both variables are inputs in eva_MatingList.txt.   
```{r, results="hide", fig.show="hide", message = FALSE, warning = FALSE}
uncer <- function(ex){
  ex_list <- list()
